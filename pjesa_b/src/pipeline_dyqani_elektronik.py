"""
Electronic Online Shop Machine Learning Pipeline

This module covers Personi B responsibilities:
...
DATA_PATH = Path("data/electronic_online_shop.csv")
...
def classification_workflow(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, GridSearchCV]]:
    ...
    for name, (pipeline, params) in model_specs.items():
        ...
        results.append(metric_dict(name, y_test, predictions, elapsed))
        save_confusion_matrix(y_test, predictions, name, labels)
...
def neural_network_grid_search(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, object]]:
    ...
    best_summary = metric_dict("Best Tuned Neural Network", y_test, predictions, elapsed)
